...
const fixtureRequiredRoles = ['fixture-image', 'fixture-audio'];
...
async function writeFixtureLock(objects = fixtureLockObjects) {
  await writeFile(
...
try {
  ...
  const verifyFixtureContract = () =>
    verifyCandidateAssetContract({
      sourceRoot,
      assetRoot,
      requiredRoles: fixtureRequiredRoles,
    });
  try {
  ...
  if (!(await rejects(verifyFixtureContract))) {
    failures.push('trusted validator accepted manifest/lock drift');
  }
  await writeFixtureManifest();
  await rm(path.join(assetRoot, assetPath));
  if (!(await rejects(verifyFixtureContract))) {
    failures.push('trusted validator accepted a missing materialized object');
  }
  await writeFile(path.join(assetRoot, assetPath), asset);
  const unexpectedPath = 'content/base/assets/images/unexpected.png';
  await writeFile(path.join(assetRoot, unexpectedPath), asset);
  if (!(await rejects(verifyFixtureContract))) {
    failures.push('trusted validator accepted unexpected local media');
  }
  await rm(path.join(assetRoot, unexpectedPath));
  await writeFixtureLock([
    ...fixtureLockObjects,
    { ...lockObject, objectKey: 'images/extra-locked.png' },
  ]);
  if (!(await rejects(verifyFixtureContract))) {
    failures.push('trusted validator accepted an extra lock object');
  }
  await writeFixtureLock();
