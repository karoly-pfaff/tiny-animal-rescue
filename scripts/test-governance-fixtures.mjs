...
for (const subject of [
  'docs(DOC-001): local assets and live epic inspection contract',
  'fix(PATCH-001): correct v0.2 asset and visual evidence',
]) {
...
for (const branch of [
  'docs/adr-0011-local-asset-materialization',
...
{
  const item = workItemFromBranch('docs/adr-0011-local-asset-materialization');
  const input = maintenanceBaseline(item, 'branch');
  input.commits[0].subject = 'feat(DOC-001): local assets and live epic inspection contract';
  record('DOC-001/reject-declared-type-drift', false, validateHistoryPolicy(input));
}
{
  const item = workItemFromBranch('docs/adr-0011-local-asset-materialization');
  ...
  record(
    'DOC-001/merge-without-fabricated-inspection',
    true,
