...
async function dimensionsFor(record) {
...
for (const record of assetRecords) {
  ...
  const recordFindings = validateAssetRecord(record, promptText, await dimensionsFor(record));
...
for (const screen of screenFiles) {
  if (pngDimensions(await readFile(screen)) === null) {
...
console.log(
  `Validated ${String(screenFiles.length)} presentation reference(s), ${String(promptFiles.length)} prompt record(s), and ${String(assetRecords.length)} inventoried production asset(s).`,
);
