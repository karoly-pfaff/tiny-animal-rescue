...
function pngHasTransparency(data) {
  if (data.length < 26 || data.subarray(1, 4).toString('ascii') !== 'PNG') {
    throw new Error('Locked visual asset is not a readable PNG.');
  }
...
function manifestRecordMatchesObject(record, object, packId) {
  const measuredMatches = object.mediaType.startsWith('image/')
    ...
    : record.durationMilliseconds === object.measured.durationMilliseconds;
  return [
    record.objectKey === object.objectKey,
    record.delivery === 'r2-locked',
    record.classification === 'production-safe',
    ...
    record.qaStatus === object.qaStatus,
    measuredMatches,
  ].every(Boolean);
}
...
async function observedPackMedia(root, packIds) {
  ...
  for (const packId of packIds) {
    const assetDirectory = path.join(root, 'content', packId, 'assets');
    const { files, findings } = await enumerateArtifactFiles(assetDirectory);
    if (findings.length > 0) throw new Error(findings.join('\n'));
    for (const file of files) {
      const relative = path.relative(root, file).replaceAll('\\', '/');
      const detected = await detectedMediaType(await readFile(file));
      if (
        mediaExtensionPattern.test(relative) ||
        /^(?:audio|image|video)\//u.test(detected ?? '')
      ) {
...
export async function verifyCandidateAssetContract({ sourceRoot, assetRoot, requiredRoles }) {
  ...
  if (findings.length > 0) throw new Error(findings.join('\n'));
  return { objectCount: plan.length };
}
